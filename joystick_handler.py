# Released by rdb under the Unlicence (unlicence.org)
...
class Joystick:
    def __init__(self, swarm_object: Union[SwarmObject, None] = None):
        ...
        if self.swarm is not None:
            self.jsl = threading.Thread(target=self.control_swarm)
            self.jsl.start()

    async def detect_joystick_state_changes(self):
        while True:
...
if __name__ == '__main__':
    js = Joystick()
    swarm = SwarmObject()
    # asyncio.get_event_loop().run_until_complete(js.detect_joystick_state_changes())
    asyncio.ensure_future(js.detect_joystick_state_changes())
